...
function renderEditorWorkspace(app: HTMLElement, initialTrip: TripDetail): void {
  let trip = initialTrip;
  let draft = detailsDraft(trip);
  let dirty = false;
  let activeSection: WorkspaceSection = "plan";
  ...
  const showDetails = (): void => {
    ...
    form.addEventListener("submit", async (event) => {
      ...
      try {
        trip = await updateTripDetails(trip.id, request);
        draft = detailsDraft(trip);
        dirty = false;
        workspaceName.textContent = trip.name;
        refreshPreview();
        ...
        summary.textContent =
          caught instanceof Error ? caught.message : "Could not save changes. Retry.";
      } finally {
        save.disabled = false;
      }
    });
  ...
  page.append(header, tabs, layout);
  app.replaceChildren(page);
  refreshPreview();
  showPlan();
}
...
export async function renderTripPlanner(
  ...
  try {
    ...
    renderEditorWorkspace(app, trip);
  } catch (caught) {
