...
function field(labelText: string, name: string, type = 'text'): HTMLLabelElement {
...
function tripCard(trip: TripSummary): HTMLLIElement {
  ...
  link.setAttribute('aria-label', `${trip.name}, ${trip.destination}, ${trip.role}`);
...
function newTripDialog(onCreated: (trip: TripSummary) => void): HTMLDialogElement {
  ...
  coordinates.append(field('Latitude', 'latitude', 'number'), field('Longitude', 'longitude', 'number'));
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      error.textContent = caught instanceof Error ? caught.message : 'Something went wrong. Please try again.';
    } finally {
      save.disabled = false;
    }
  });
...
export async function renderMyTrips(app: HTMLElement): Promise<void> {
  ...
  try {
    ...
    status.textContent = caught instanceof Error ? caught.message : 'Something went wrong. Please try again.';
  }
}


export async function renderSelectedTrip(app: HTMLElement, tripId: string): Promise<void> {
  document.documentElement.style.scrollSnapType = 'none';
  ...
  try {
    ...
    loading.textContent = caught instanceof Error ? caught.message : 'Could not load this trip.';
  }
}
